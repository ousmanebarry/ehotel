						...
						<Table>
							...
							<tbody>
								{props.capacitiesInfo
									.filter((capacityInfo) => (props.hotel_id ? capacityInfo.hotel_id === props.hotel_id : true))
									.map((capacityInfo) => (
										<tr key={`${capacityInfo.hotel_chain_name}-${capacityInfo.hotel_id}-${capacityInfo.room_number}`}>
											<td>{capacityInfo.hotel_chain_name}</td>
											<td>
												<Link href={`/hotels?hotel_id=${capacityInfo.hotel_id}`}>{capacityInfo.hotel_id}</Link>
											</td>
...
export const getServerSideProps: GetServerSideProps<HotelsProps> = async (context) => {
	...
	if ('hotel_id' in context.query) {
		...
		const { data } = await axios.get<CapacityInfo[]>(`${process.env.NEXT_PUBLIC_URL}/hotel/hotel/total_capacity`);
