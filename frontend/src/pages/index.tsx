...
import { Container, Text, Center, Loader, Flex } from '@mantine/core';
...
export default function Home() {
	const { startDate, endDate, price, hotelChain, category, roomCapacity, location } = useSearchQuery((state) => state);
	const debounceState = useRef<null | number>(null);
	const [result, setResult] = useState<HotelChainSearch[]>([]);
	const [loading, setLoading] = useState(true);
	const firstRender = useRef(true);
	const [hotelChains, setHotelChains] = useHotelChains((state) => [state.hotelChains, state.setHotelChains]);

	useEffect(() => {
		axios.get<HotelChainInfo[]>(`${process.env.NEXT_PUBLIC_URL}/hotel_chain/hotel_chain`).then((res) => {
			const { data } = res;
	...
	useEffect(() => {
		...
		debounceState.current = setTimeout(() => {
			...
			axios
				.get<HotelChainSearch[]>(`${process.env.NEXT_PUBLIC_URL}/hotel/hotel/search`, {
				...
				<Container sx={{ marginTop: '20px', marginBottom: '20px' }}>
					<Center>
						<Flex wrap='wrap' gap='30px'>
							...
							{result.length === 0 && !loading && <Text>No rooms available with the given criteria</Text>}
