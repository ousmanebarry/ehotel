...
import { TextInput, PasswordInput, Button, Text, Stack, Flex, Group, NumberInput, Radio } from '@mantine/core';
...
export default function SignUp() {
	...
	const handleSubmit = form.onSubmit(async (info) => {
		try {
			...
			const nextRes = await axios.post(`${process.env.NEXT_PUBLIC_URL}/auth/login`, {
	...
	return (
		<Flex sx={{ height: '100%', marginTop: '20px' }} justify='center' align='center'>
			<Stack spacing='md' sx={{ padding: '40px' }}>
				<NumberInput placeholder='SSN' label='SSN' {...form.getInputProps('ssn')} />

				<Group position='apart'>
					<NumberInput placeholder='Hotel ID' label='Hotel ID' {...form.getInputProps('hotelId')} />
					<NumberInput placeholder='Employee ID' label='Employee ID' {...form.getInputProps('employeeId')} />
				</Group>
				<Group position='apart'>
					<TextInput placeholder='Role' label='Role' {...form.getInputProps('role')} />
					<Radio.Group name='Is Manager' label='Is Manager' {...form.getInputProps('isManager')}>
						<Group mt='xs'>
				...
				<Group position='apart'>
					<TextInput placeholder='Street Name' label='Street Name' {...form.getInputProps('streetName')} />
					<NumberInput placeholder='Street Number' label='Street Number' {...form.getInputProps('streetNumber')} />
				</Group>
				<TextInput placeholder='City' label='City' {...form.getInputProps('city')} />
				<Group position='apart'>
					<TextInput placeholder='Country' label='Country' {...form.getInputProps('country')} />
					<TextInput placeholder='Region' label='Region' {...form.getInputProps('region')} />
				</Group>
