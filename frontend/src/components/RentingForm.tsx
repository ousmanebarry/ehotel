...
import { Button, Stack, Group, NumberInput, Text, Center } from '@mantine/core';
...
export default function RentingForm(props: HotelForm) {
	...
	const form = useForm({
		...
		validate: {
			discount: isInRange({ min: 0, max: 100 }, 'Discount must be between 0 to 100'),
			additional_charges: isInRange({ min: 0 }, 'Additional charges must be positive'),
	...
	return (
		<Stack spacing='md'>
			<NumberInput placeholder='Customer SSN' label='Customer SSN' {...form.getInputProps('customer_SSN_SIN')} />
			<Group position='apart'>
				<NumberInput placeholder='Hotel Id' label='Hotel Id' {...form.getInputProps('hotel_ID')} />
				<NumberInput placeholder='Room number' label='Room number' {...form.getInputProps('room_number')} />
			</Group>
			<Group position='apart'>
				<NumberInput
			...
			<Center>
				<Stack sx={{ padding: '10px 0px' }}>
					<DateRangePicker setDateRange={handleDateChange} startDateLabel='Check In' endDateLabel='Check Out' />
			...
			<Group position='apart'>
				...
				<Button type='submit' onClick={form.onSubmit(props.onSubmit as any) as any} disabled={!opened}>
					Submit
				</Button>
			</Group>
		</Stack>
