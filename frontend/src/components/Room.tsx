...
import { Paper, Text, Group, Rating, Modal, Stack, Button, Box } from '@mantine/core';
...
export default function Room(props: RoomProps) {
	...
	const [startDate, endDate] = useSearchQuery((state) => [state.startDate, state.endDate]);
	...
	const handleDelete = async () => {
		try {
			...
			await axios.delete(`${process.env.NEXT_PUBLIC_URL}/room/room/${props.hotel.hotel_ID}/${props.room.room_number}`, {
			...
			<Stack spacing='sm'>
				...
				<Group position='apart'>
					<Text>
						...
						{props.hotelChains.find((hotelChain) => hotelChain.chain_ID === props.hotel.chain_id)?.name}
					</Text>
				...
				{token.role === 'customer' && (
					<Button size='md' compact onClick={handleBooking} disabled={!startDate && !endDate}>
						Book Now
					</Button>
				)}
				{token.role === 'employee' && (
